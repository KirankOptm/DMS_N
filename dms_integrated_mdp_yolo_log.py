...
try:
    ...
    print("[WARNING] gaze_tracker.py not found - gaze tracking disabled")
...
_buzzer_thread = None  # Track active buzzer thread for non-blocking operation
...
def _check_buzzer():
    ...
    if _BUZZER_AVAILABLE is None:
        try:
            ...
            print(f"[Buzzer] ✓ Hardware available: {BUZZER}")
        except Exception as e:
            _BUZZER_AVAILABLE = False
            print(f"[Buzzer] ✗ Hardware not available: {e}")
...
def _buzzer_beep_worker(times, on_s, off_s):
    """Background worker for buzzer beeps (non-blocking)"""
...
def buzzer_beep(times=3, on_s=0.05, off_s=0.05):
    """
    Beep buzzer N times (NON-BLOCKING via thread)
    
    CRITICAL: Runs in background thread to avoid blocking main loop.
    This ensures INSTANT return to frame processing (~0ms delay).
    Default: 5 beeps × 100ms = 500ms total (runs in parallel with processing)
    """
    ...
    # Stop any previous beep sequence (prevents overlap)
    if _buzzer_thread and _buzzer_thread.is_alive():
        return  # Skip if already beeping
    
    # Start new beep thread (daemon=True for auto-cleanup on exit)
    _buzzer_thread = _threading.Thread(
        target=_buzzer_beep_worker,
        args=(times, on_s, off_s),
        daemon=True
    )
    _buzzer_thread.start()
    
    # Return IMMEDIATELY (0ms delay - buzzer runs in background)
    print(f"[Buzzer] ✓ Beeping {times}x in background (non-blocking)")
...
try:
    ...
    print("[WARNING] authenticate_face_board.py not found - authentication disabled")
    _HAS_AUTH = False
...
def hand_near_ear(landmarks, hand_landmarks, w, h) -> bool:
    ...
    Asymmetric thresholds for RIGHT-side 45° camera (10cm above driver):
    - Right ear (CLOSER to camera): 20% radius RELAXED for phone detection
    - Left ear (FARTHER from camera): 60% radius for perspective distortion
    """
    try:
    ...
    # Asymmetric radius for RIGHT-side 45-degree camera (10cm above)
    r_pix_right = 0.20 * face_h  # Right ear (CLOSER to camera) - RELAXED for phone call detection
    r_pix_left = 0.60 * face_h   # Left ear (FARTHER from camera) - larger radius for perspective distortion
    
    # Use nose as filter boundary - RIGHT ear more permissive (natural phone position), LEFT ear relaxed (distant)
    nose_filter_right = nose_y + (0.12 * face_h)  # Relaxed for right ear (allow phone slightly below nose)
    nose_filter_left = nose_y + (0.15 * face_h)  # Relaxed for left ear (allow below nose for side angle)
    ...
    for idx in tips:
        ...
        # Right ear: relaxed detection (closer to camera, more forgiving for phone call)
        if hy <= nose_filter_right:
            if np.hypot(hx - ear_r[0], hy - ear_r[1]) <= r_pix_right:
                near_r += 1
        
        # Left ear: allow below nose (farther from camera, angled view needs tolerance)
...
class YOLOWorker(_threading.Thread):
    def __init__(self, model_path: str, imgsz: int, conf: float, iou: float,
        ...
        if (self.yolo is None) and _HAS_ORT and _os.path.isfile(model_path) and model_path.lower().endswith('.onnx'):
            try:
                dms_print_global("YOLOWorker", f"Loading ONNX model: {model_path}")
                ...
                dms_print_global("YOLOWorker", "ONNX session created successfully")
                dms_print_global("YOLOWorker", f"Input name: {self.ort_input_name}, Input shape: {self.ort_input_shape}")
            except Exception as e:
    ...
    def _infer_onnx(self, frame):
        ...
        # Debug: print frame shape during first inference
        if not hasattr(self, '_debug_printed'):
            print(f"[YOLOWorker._infer_onnx] Input frame shape: {img0.shape}, Target size: {img_size} (from model metadata)")
        ...
        if not hasattr(self, '_debug_printed2'):
            print(f"[YOLOWorker._infer_onnx] Preprocessed input shape: {x.shape}, dtype: {x.dtype}")
            self._debug_printed2 = True
        outputs = self.ort_session.run(None, {self.ort_input_name: x})
        if not hasattr(self, '_debug_printed3'):
            print(f"[YOLOWorker._infer_onnx] Inference successful! Output shapes: {[o.shape for o in outputs]}")
...
def main():
    ...
    ap.add_argument('--ear_threshold', type=float, default=0.165)
    ...
    ap.add_argument('--mar_threshold', type=float, default=0.65)
    ...
    ap.add_argument('--head_turn_threshold', type=float, default=0.22, help='Threshold for head turn detection (0.22 = optimized for 45° side camera 10cm above, 0.08 = strict for front cameras)')
    ...
    # AI Model Enable/Disable Controls (8 combinations support)
    ap.add_argument('--disable_face_recognition', action='store_true', help='Disable face recognition/authentication at startup (overrides --enable_auth)')
    ap.add_argument('--disable_facelandmark', action='store_true', help='Disable MediaPipe face tracking (no drowsiness/distraction detection)')
    ap.add_argument('--disable_seatbelt', action='store_true', help='Disable YOLO model (no seatbelt/phone/cigarette detection)')
    ...
    if getattr(args, 'remote_server', None):
        ...
        if no_stream_mode:
            dms_print("DMS", f"Remote mode (NO STREAM): Logs only to {args.remote_server}:{getattr(args, 'remote_port', 5000)}")
            dms_print("DMS", "Video frames will NOT be sent (--no_stream enabled)")
            ...
            dms_print("DMS", f"Remote mode: Sending frames to {args.remote_server}:{getattr(args, 'remote_port', 5000)}")
            if not frame_sender.connect():
                dms_print("DMS", "WARNING: Could not connect to remote server. Will retry...")
    else:
        if no_stream_mode:
            dms_print("DMS", "Logs-only mode (--no_stream enabled, no remote server specified)")
        else:
            dms_print("DMS", "Local display mode (use --remote_server to enable TCP streaming)")

    # Driver Authentication (if enabled)
    authenticated_driver = None
    driver_id = None
    
    # Check if face recognition is disabled (overrides --enable_auth)
    face_recognition_disabled = getattr(args, 'disable_face_recognition', False)
    
    if getattr(args, 'enable_auth', False) and _HAS_AUTH and not face_recognition_disabled:
        dms_print("Authentication", "Driver authentication enabled")
        try:
            ...
            if auth_success:
                ...
                driver_id = auth_id
                dms_print("Authentication", f"✓ Driver authenticated: {auth_name} (ID: {auth_id})")
            else:
                dms_print("Authentication", "⚠ Proceeding in unauthorized mode")
            
            # Force cleanup and free memory
            import gc
            gc.collect()
            dms_print("Authentication", "Models unloaded - starting DMS monitoring")
            
        except Exception as e:
            dms_print("Authentication", f"✗ Authentication failed: {e}")
            dms_print("Authentication", "Proceeding in unauthorized mode")
    else:
        if face_recognition_disabled:
            dms_print("Authentication", "Face Recognition DISABLED (--disable_face_recognition)")
        else:
            dms_print("Authentication", "Driver authentication disabled (use --enable_auth to enable)")
    
    # Production Configuration Summary
    dms_print("=== PRODUCTION CONFIG ===", "")
    dms_print("Threading", "YOLO: Background Thread (non-blocking)")
    dms_print("Threading", "MediaPipe: Main Thread (stable, low latency)")
    dms_print("Camera", f"GStreamer: {'ENABLED (hardware accel)' if getattr(args, 'use_gstreamer', True) else 'DISABLED (OpenCV fallback)'}")
    dms_print("Alerts", "Console: INSTANT (0ms) → Screen: +500ms delay")
    if authenticated_driver:
        dms_print("Driver", f"{authenticated_driver} (ID: {driver_id})")
    dms_print("========================", "")
    
    # AI Models Status Summary
    facelandmark_disabled = getattr(args, 'disable_facelandmark', False)
    seatbelt_disabled = getattr(args, 'disable_seatbelt', False)
    
    dms_print("===== AI MODELS STATUS =====", "")
    if face_recognition_disabled:
        dms_print("AI Models", "Face Recognition: DISABLED")
    elif authenticated_driver:
        dms_print("AI Models", f"Face Recognition: ENABLED - Authenticated as {authenticated_driver}")
    else:
        dms_print("AI Models", "Face Recognition: Not used (--enable_auth not set)")
    
    if facelandmark_disabled:
        dms_print("AI Models", "MediaPipe FaceMesh: DISABLED (no drowsiness detection)")
    else:
        dms_print("AI Models", "MediaPipe FaceMesh: ENABLED (drowsiness/distraction detection)")
    
    if seatbelt_disabled:
        dms_print("AI Models", "YOLO Detection: DISABLED (no seatbelt/phone/cigarette)")
    else:
        dms_print("AI Models", "YOLO Detection: ENABLED (seatbelt/phone/cigarette detection)")
    
    dms_print("===============================", "")
    
    # MediaPipe solutions - conditional initialization based on --disable_facelandmark flag
    if facelandmark_disabled:
        face_mesh, hands, pose, mp_drawing, hand_connections, face_mesh_connections = None, None, None, None, None, None
        if not getattr(args, 'quiet', False):
            dms_print("MediaPipe", "DISABLED - Skipping initialization (--disable_facelandmark)")
    else:
        face_mesh, hands, pose, mp_drawing, hand_connections, face_mesh_connections = create_solutions()
        if not getattr(args, 'quiet', False):
            dms_print("MediaPipe", "Using inline processing (production-stable)")
    
    dms_print("Calibration", f"Starting {args.calibration_time}s calibration period...")
    dms_print("Calibration", "Look straight ahead and keep face visible")
    ...
    # Check if YOLO is disabled
    if seatbelt_disabled:
        if not args.quiet:
            dms_print("YOLO", "DISABLED - Skipping initialization (--disable_seatbelt)")
    else:
        for p in candidate_paths:
            ...
            if worker.validate_model():
                ...
                selected_model = p
                if not args.quiet:
                    dms_print("YOLO", f"Using model: {p}")
                yolo_worker.start()
                break
            else:
    ...
    dms_print("Camera", f"Requested device: {args.camera_device}")
    ...
    try:
        ...
        dms_print("Camera", f"Opening camera {device_num}.")
        ...
        if use_gstreamer:
            try:
                ...
                dms_print("Camera", "Trying GStreamer (RAW YUYV 640x480@30).")
                dms_print("Camera", f"Pipeline: {gst_pipeline}")
                ...
                if cap is not None and cap.isOpened():
                    ...
                    dms_print("Camera",
                            f"GStreamer opened={cap.isOpened()}, first_read_ok={ok}, "
                            f"frame={'None' if frame is None else frame.shape}")

                    if ok and frame is not None:
                        dms_print("Camera", "✓ GStreamer opened successfully (RAW YUYV path)")
                        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
                        # If your code needs this first frame later, store it; otherwise ignore.
                    else:
                        dms_print("Camera", "GStreamer opened but cannot read frames -> fallback.")
                        cap.release()
                        cap = None
                else:
                    dms_print("Camera", "GStreamer open failed -> fallback.")
                    cap = None

            except Exception as e:
                dms_print("Camera", f"GStreamer failed: {e}, falling back to OpenCV.")
                cap = None
        # ---------- END UPDATED GStreamer block ----------

        # Fallback to OpenCV V4L2 if GStreamer failed or disabled
        if cap is None or not cap.isOpened():
            dms_print("Camera", "Using OpenCV V4L2 backend.")
            ...
            ok, frame = cap.read()
            dms_print("Camera", f"V4L2 first_read_ok={ok}, frame={'None' if frame is None else frame.shape}")
        ...
        dms_print("Camera", "Opened successfully")
    ...
    buzzer_triggered = False      # Track if buzzer already fired for this closure event (KSS 9.0)
    kss8_buzzer_triggered = False # Track if buzzer already fired for KSS 8.0 (head droop)
    ...
    # Gaze tracking (RIGHT-side 45° camera, 10cm above driver - CUSTOM CENTER)
    gaze_tracker = None
    if _HAS_GAZE_TRACKER:
        gaze_tracker = GazeTracker(
            horizontal_threshold=0.14,  # Tolerance around center (wider for side angle)
            vertical_threshold=0.18,    # Tolerance for vertical (camera above eye level)
            horizontal_center=0.38,     # RIGHT-side camera: shifted right (was 0.35)
            vertical_center=0.58,       # Camera 10cm above: shifted up slightly (was 0.60)
            smoothing_frames=2,         # Faster response
            flip_horizontal=True,       # Flip for natural left/right mapping
            flip_vertical=False         # Don't flip vertical
        )
        if not facelandmark_disabled:
            dms_print("Gaze", "Eye gaze tracking enabled (RIGHT-side 45° camera compensated)")
    ...
    # NEW: Require sustained hand-near-ear for call detection
    mobile_call_start_time = None  # Track when hand first went near ear
    MOBILE_CALL_DURATION = 2.5  # 2.5 seconds sustained = confirmed call (reduced from 5s for right-side camera)
    ...
    def add_alert(message: str):
        ...
        if "NO SEATBELT" in message and buzzer_count < BUZZER_MAX_COUNT:
            ...
            buzzer_count += 1
            dms_print("Buzzer", f"NO SEATBELT beep #{buzzer_count}/{BUZZER_MAX_COUNT}")
        
        # Reset buzzer count when seatbelt is worn
        elif "Seatbelt Worn" in message:
            if buzzer_count > 0:
                dms_print("Buzzer", "Seatbelt worn - buzzer reset")
            buzzer_count = 0
    ...
    if not facelandmark_disabled:
        dms_print("KSS", "✓ AIS 184 drowsiness scoring initialized")
    ...
    while cap.isOpened():
        ...
        ret, frame = cap.read()
        if not ret:
            dms_print("WARNING", "Failed to read frame")
            break
        ...
        if args.crop_center > 0 and args.crop_center <= 0.5:
            ...
            if fid == 1:
                    dms_print("Camera", f"Cropped from {w_orig}x{h_orig} to {frame.shape[1]}x{frame.shape[0]}")
            dms_print("Camera", f"Frame resolution: {frame.shape[1]}x{frame.shape[0]}")
            dms_print("Camera", f"Frame format: {frame.dtype}")
            dms_print("DMS", "Processing started...")
        ...
        if yolo_worker and should_run_yolo:
            ...
            if fid % 30 == 0:  # Log every 30 frames to avoid spam
                dms_print("YOLO", f"Running detection (every {YOLO_INTERVAL}s)")
        ...
        if gaze_tracker and result and result.multi_face_landmarks:
            ...
            # Print gaze direction when it changes (with FLIPPED ratios for debugging)
            if gaze_dir != "CENTER":
                # Calculate flipped ratios for display
                h_flipped = 1.0 - h_ratio
                if gaze_dir == "LEFT":
                    print(f"[Gaze] Looking LEFT (Raw:{h_ratio:.2f} Flip:{h_flipped:.2f} V:{v_ratio:.2f})")
                elif gaze_dir == "RIGHT":
                    print(f"[Gaze] Looking RIGHT (Raw:{h_ratio:.2f} Flip:{h_flipped:.2f} V:{v_ratio:.2f})")
                elif gaze_dir == "UP":
                    print(f"[Gaze] Looking UP (Raw V:{v_ratio:.2f} H:{h_ratio:.2f}→{1.0-h_ratio:.2f})")
                elif gaze_dir == "DOWN":
                    print(f"[Gaze] Looking DOWN (Raw V:{v_ratio:.2f} H:{h_ratio:.2f}→{1.0-h_ratio:.2f})")
            # Debug: Print raw ratios every 15 frames to diagnose UP threshold
            elif fid % 15 == 0:
                h_flipped = 1.0 - h_ratio
                print(f"[Gaze Debug] CENTER - H:{h_ratio:.2f}→{h_flipped:.2f} V:{v_ratio:.2f} [UP<0.48 DOWN>0.52 L<0.44 R>0.56]")
        ...
        if fid % 30 == 0:
            ...
            dms_print("Frame", f"{face_status}, {hand_status}", fid)
        ...
        # Face lost timer logic (like DMSv8) - only when MediaPipe is enabled
        if face_mesh is not None:
            if not (result and getattr(result, 'multi_face_landmarks', None)):
                if 'face_lost_start_time' not in globals() or globals().get('face_lost_start_time') is None:
                    ...
                    dms_print("WARNING", "Face lost (likely head turn) - starting timer")
                else:
        ...
        if result and getattr(result, 'multi_face_landmarks', None):
                ...
                if 'face_lost_start_time' in globals() and globals().get('face_lost_start_time') is not None:
                    dms_print("INFO", "Face re-acquired")
                ...
                # OPTIMIZED EAR SMOOTHING for critical eye closure detection
                # Uses adaptive smoothing: fast response for closure, slower for opening (reduce false positives)
                if 'smoothed_ear' not in globals():
                    globals()['smoothed_ear'] = raw_avg_ear
                else:
                    # Adaptive smoothing factor based on closure direction
                    if raw_avg_ear < globals()['smoothed_ear']:  # Eyes closing (EAR decreasing)
                        SMOOTH_FACTOR = 0.6  # FAST response (60% new, 40% old) - was 0.3
                    else:  # Eyes opening (EAR increasing)
                        SMOOTH_FACTOR = 0.3  # Slower (30% new, 70% old) - reduce jitter/false clears
                ...
                if fid % 10 == 0:
                    dms_print("Eye Openness", f"{current_eye_openness:.1f}% (EAR: {avg_ear:.3f})")
                ...
                # Eye closure duration tracking (CRITICAL: Use EAR-only for timer - don't wait for iris)
                # For side camera (45°), iris detection can be unreliable, causing timer resets
                # OPTIMIZED: Use raw_avg_ear for reset check to avoid smoothing lag
                raw_eye_closed = raw_avg_ear < args.ear_threshold  # Raw check (no smoothing delay)
                
                if eye_closed_by_ear:  # Smoothed EAR for stable detection
                    if eye_close_start_time is None:
                        eye_close_start_time = now_eye
                        print(f"[EyeTimer] Eyes CLOSED at frame {fid} (time={now_eye:.3f}, raw={raw_avg_ear:.3f}, smoothed={avg_ear:.3f})")
                    
                    # Calculate duration for display alerts (local use only)
                    current_duration = now_eye - eye_close_start_time
                    
                    # Alert on duration milestones (using local duration for display)
                    if current_duration >= 3.0:
                        add_alert(f"Eye Closure Duration: {current_duration:.1f}s - CRITICAL")
                    elif current_duration >= 2.0:
                        add_alert(f"Eye Closure Duration: {current_duration:.1f}s - WARNING")
                    elif current_duration >= 1.0 and int(current_duration * 10) % 5 == 0:
                        add_alert(f"Eye Closure Duration: {current_duration:.1f}s")
                    
                    # Print level when eyes are closed (only if iris also confirms - for display accuracy)
                    if fid % 5 == 0 and iris_missing_or_low:
                        add_alert(f"Eye Closure Level: {closure_level} ({current_eye_openness:.1f}%)")
                
                # CRITICAL FIX: Use RAW EAR for reset check (faster response, no smoothing lag)
                # Check BOTH smoothed closed AND raw opened to ensure clean reset
                if not eye_closed_by_ear or (not raw_eye_closed and eye_close_start_time is not None):
                    # Eyes opened - reset ALL timers (uses raw EAR to detect opening faster)
                    if eye_close_start_time is not None:
                        final_duration = now_eye - eye_close_start_time
                        if final_duration > 0.5:
                            add_alert(f"Eyes Reopened (was closed for {final_duration:.1f}s)")
                            print(f"[EyeTimer] Eyes OPENED at frame {fid} (time={now_eye:.3f}, duration={final_duration:.1f}s, raw={raw_avg_ear:.3f})")
                        
                        # FORCE RESET smoothed_ear to speed up next detection cycle
                        if raw_avg_ear > args.ear_threshold + 0.02:  # Clear margin above threshold
                            globals()['smoothed_ear'] = raw_avg_ear  # Reset to raw value (no lag)
                            print(f"[EyeTimer] Smoothed EAR RESET to {raw_avg_ear:.3f} (was {avg_ear:.3f})")
                ...
                if calibration_mode:
                    ...
                    if fid % 30 == 0:
                        dms_print("GAZE", f"deviation={gaze_deviation:.3f}, center={gaze_center:.3f}, L_streak={gaze_left_confirm_streak}, R_streak={gaze_right_confirm_streak}")
                    ...
                    if fid % 30 == 0:
                        dms_print("HeadPos", f"Y_signed={sm_head_y_signed:.3f}, Y_offset={yoff:.3f}")
        ...
        # ========== KSS CALCULATION (ONLY when MediaPipe is enabled) ==========
        if not facelandmark_disabled:
            ...
            try:
                ...
                if eye_close_duration > 1.5:
                    ...
                    print(f"[KSS Override TRIGGERED] eye_close_duration={eye_close_duration:.2f}s > 1.5s at frame {fid}")
                    dms_print("KSS Override", f"🚨 CRITICAL: Eyes closed {eye_close_duration:.1f}s → KSS=9.0 (DANGER)")
                ...
                # PRIORITY 2: Head droop overrides (only if eyes not already critical)
                    ...
                    if droop_duration_direct <= 2.0:
                        ...
                        kss_confidence = 0.80
                        override_triggered = True
                        dms_print("KSS Override", f"WARNING: Head nod {droop_duration_direct:.1f}s → KSS=7.0")
                    # Rule 2: Sustained head droop >2 seconds
                        ...
                        if avg_ear < 0.18:
                            ...
                            kss_confidence = 0.95
                            override_triggered = True
                            dms_print("KSS Override", f"CRITICAL: Eyes closed + Head droop {droop_duration_direct:.1f}s → KSS=9.0")
                        # Path 2: Sustained head droop alone (eyes may be open) → KSS 8.0
                        else:
                            kss_score = 8.0  # Critical - head droop alone
                            kss_confidence = 0.85
                            override_triggered = True
                            dms_print("KSS Override", f"CRITICAL: Head droop {droop_duration_direct:.1f}s → KSS=8.0")
                            
                            # BUZZER for KSS 8.0 (3 beeps) - ONCE ONLY per episode
                            if not kss8_buzzer_triggered:
                                buzzer_beep(times=3, on_s=0.05, off_s=0.05)
                                kss8_buzzer_triggered = True
                                print(f"[Buzzer] KSS 8.0 TRIGGER - Head droop {droop_duration_direct:.1f}s (3 beeps)")
                
                # If no override, mark t_kss after normal calculation
                if not override_triggered and _LAT:
                    lat.mark('t_kss')
                
                # ========== KSS >= 7.0 LATENCY LOGGING (Print immediately when threshold hit) ==========
                # Log latency IMMEDIATELY when KSS >= 7.0, don't wait for alert manager
                if _LAT and kss_score >= 7.0 and not kss7_latency_logged:
                    lat.mark('t_buzzer')  # Mark buzzer timing
                    print(f"\n{'='*70}")
                    print(f"[KSS_7.0_LATENCY] Frame {fid} - KSS Score: {kss_score:.1f}/9")
                    print(f"{'='*70}")
                    lat.print_kss7_latency(fid, kss_score)
                ...
                # Reset KSS 8.0 buzzer flag when head droop ends (score drops below 8.0)
                if kss_score < 8.0 and kss8_buzzer_triggered:
                    kss8_buzzer_triggered = False
                    print(f"[Buzzer] KSS 8.0 flag reset - head droop ended")
                ...
                if kss_alert:
                    ...
                    if _LAT and override_triggered and eye_close_duration > 1.5 and not eye_closure_latency_logged:
                        print(f"[DEBUG] About to log latency: _LAT={_LAT}, override={override_triggered}, duration={eye_close_duration:.1f}, logged={eye_closure_latency_logged}")
                        lat.log_kss_pipeline(fid)
                        eye_closure_latency_logged = True
                        print(f"[DEBUG] Latency logged successfully!")
        ...
        # ========== END OF KSS CALCULATION BLOCK ==========

    # YOLO-driven alerts (works independently of MediaPipe)
        if dets:
            def rect_iou(a, b):
                        ...
                        if seatbelt_state == 'no':
                            if (now_tt - last_alert_times['NO SEATBELT'] >= 0.8):
                                if no_seatbelt_print_count < SEATBELT_MAX_ALERTS:
                                    ...
                                    dms_print("Seatbelt", f"NO SEATBELT confirmed ({no_seatbelt_alert_count}/{SEATBELT_MAX_ALERTS})")
