...
class FrameSender:
    ...
    def close(self):
        """Close connection - ultra-defensive to prevent crashes"""
        self.connected = False
        if self.socket is not None:
            try:
                self.socket.close()
            except:
                pass  # Silently ignore any close errors
            finally:
                self.socket = None
    ...
    if getattr(args, 'remote_server', None):
        ...
        try:
            ...
            if not frame_sender.connect():
                ...
                # Don't set to None - let it retry in main loop
        except Exception as e:
            dms_print("DMS", f"ERROR: Failed to initialize remote sender: {e}")
            frame_sender = None
    else:
        dms_print("DMS", "Local display mode")
    ...
    while cap.isOpened():
        ...
        # Periodic re-emit of seatbelt status (DISABLED - trigger once only)
            ...
            if frame_sender is not None:
                # Try remote TCP send
                try:
                    if frame_sender.send_frame(frame):
                        display_success = True
                    else:
                        print("[FrameSender] ⚠ Send failed, switching to local display")
                        frame_sender = None
                except Exception as e:
                    print(f"[FrameSender] Exception: {e}, switching to local display")
                    frame_sender = None
            
            # Fallback to local display if remote failed or not configured
            if not display_success:
                try:
                    cv2.imshow("DMS Integrated (refactored)", frame)
                except Exception:
                    pass  # Silently skip display on headless systems
            
            # Always call waitKey to process events (prevents crashes)
            try:
                key = cv2.waitKey(1) & 0xFF
                if key == 27 or key == ord('q'):
                    break
            except Exception:
                pass  # Ignore waitKey errors on headless systems
