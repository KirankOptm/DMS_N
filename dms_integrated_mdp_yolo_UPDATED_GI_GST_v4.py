...
# --- Optional: GStreamer capture via GI (recommended on i.MX93 when OpenCV is built without GStreamer) ---
_HAS_GI_GST = False
try:
...
class _GstGiCapture:
    """Minimal camera capture wrapper using GStreamer GI + appsink.
    Exposes .read(), .isOpened(), .release() like OpenCV VideoCapture.
    Also keeps .last_frame_age_ms (staleness) for profiling / customer proof.
    """
    ...
    def _start(self):
        if not _HAS_GI_GST:
            raise RuntimeError("GI GStreamer not available")
    ...
    def read(self):
        if not self._opened or self.appsink is None or self.pipeline is None:
            return False, None
        sample = None
        try:
            sample = self.appsink.emit("try-pull-sample", int(1 * Gst.SECOND))
        except Exception:
            try:
        ...
        try:
            import numpy as _np
            frame = _np.frombuffer(mapinfo.data, dtype=_np.uint8).reshape((height, width, 3)).copy()
            try:
                clk = self.pipeline.get_clock()
                base = self.pipeline.get_base_time()
                now_rt = int(clk.get_time() - base)
                pts = int(buf.pts)
                if pts != Gst.CLOCK_TIME_NONE and now_rt >= 0:
                    self.last_frame_age_ms = (now_rt - pts) / 1e6
            except Exception:
                pass
            return True, frame
        finally:
...
class FrameSender:
    """
    Hardened TCP frame sender (no-crash + low-latency).
    Protocol: 4-byte big-endian length prefix + JPEG bytes.
    - connect(): starts background sender thread
    - send_frame(frame): encodes and queues (drops old frames if busy)
    - close(): stops thread + closes socket
    """
    def __init__(self, host, port, jpeg_quality=75, max_queue=2):
    ...
    def connect(self) -> bool:
        """Start the worker thread and wait briefly for initial connection."""
        if self._t.is_alive():
    ...
    def send_frame(self, frame_bgr) -> bool:
        """Encode BGR frame to JPEG and queue for sending. Returns True if queued."""
        if self._stop_evt.is_set():
            return False
        if not self.connected:
            # don't block; try to connect in background if not started
            try:
                self.connect()
            except Exception:
                return False
        ...
        try:
            ...
            return self._enqueue(header + payload)
        except Exception:
            return False

    def _enqueue(self, payload: bytes) -> bool:
        """Non-blocking enqueue. Drops oldest if full."""
        if not self.connected or self._stop_evt.is_set():
            return False
        try:
            if self._q.full():
                try:
    ...
    def _connect(self) -> bool:
        ...
        try:
            ...
            self.sock = s
            self.connected = True
            print(f"[FrameSender] ✓ Connected to {self.host}:{self.port}")
            return True
        except Exception as e:
            print(f"[FrameSender] ERROR connect failed: {e}")
            self.connected = False
            try:
                if self.sock:
                    self.sock.close()
    ...
    def _worker(self):
        ...
        try:
            if not self._connect():
                # Don't crash the app if remote is unavailable
                return
            ...
            while not self._stop_evt.is_set():
                ...
                try:
                    ...
                    # Remote closed or network issue: log and exit thread safely
                    print(f"[FrameSender] Connection closed / send failed: {e}")
                    self.connected = False
                    try:
                        if self.sock:
                            self.sock.close()
            ...
            print(f"[FrameSender] FATAL thread exception: {e}")
            traceback.print_exc()
        finally:
            self.connected = False
            try:
...
# Global DMS print function (set by main())
_dms_print_func = None

def set_dms_print(func):
    """Register the DMS print function for use by classes"""
    global _dms_print_func
    _dms_print_func = func

def dms_print_global(category, message, frame_id=None):
    """Global DMS print function callable from classes"""
    if _dms_print_func is not None:
...
def main():
    ...
    if getattr(args, 'enable_auth', False) and _HAS_AUTH:
        ...
        dms_print("Authentication", "Driver authentication disabled")
    ...
    # Camera initialization - Prefer GI GStreamer capture (low-latency), fallback to OpenCV V4L2
    ...
    try:
        ...
        use_gstreamer = bool(getattr(args, 'use_gstreamer', True))  # Default TRUE
        cap = None

        # IMPORTANT: On your i.MX93 build, OpenCV reports "GStreamer: NO".
        # So cv2.CAP_GSTREAMER pipelines will always fail. We use GI (python-gst) instead.
        if use_gstreamer and _HAS_GI_GST:
            try:
                gst_pipeline = (
                    f"v4l2src device=/dev/video{device_num} io-mode=2 ! "
                ...
                dms_print("Camera", "Trying GI GStreamer (RAW YUYV 640x480@30).")
                dms_print("Camera", f"Pipeline: {gst_pipeline}")

                cap = _GstGiCapture(gst_pipeline)
                ...
                dms_print("Camera",
                          f"GI_GStreamer opened={cap.isOpened()}, first_read_ok={ok}, "
                          f"frame={'None' if frame is None else frame.shape}")

                if not (ok and frame is not None):
                    dms_print("Camera", "GI GStreamer opened but cannot read frames -> fallback.")
                    cap.release()
                    cap = None
                else:
                    dms_print("Camera", "✓ GI GStreamer capture enabled (low-latency)")
            except Exception as e:
                dms_print("Camera", f"GI GStreamer failed: {e} -> fallback to OpenCV.")
                try:
                    if cap is not None:
                        cap.release()
                except Exception:
                    pass
                cap = None
        elif use_gstreamer and not _HAS_GI_GST:
            dms_print("Camera", "GI GStreamer not available -> using OpenCV fallback.")
        else:
            dms_print("Camera", "GStreamer disabled -> using OpenCV fallback.")

        if cap is None or not cap.isOpened():
            dms_print("Camera", "Using OpenCV V4L2 backend (may be higher latency).")
    ...
    # YOLO Cycling: Detect continuously, alert 2 times, then sleep 60 seconds
    yolo_active = True
    yolo_cycle_start = time.time()
    YOLO_SLEEP_DURATION = 30.0   # Sleep for 30 seconds after 2 alerts
    yolo_sleep_until = 0.0  # Timestamp when sleep ends
    ...
    kss_calculator, kss_alert_manager = create_kss_system(buzzer_callback=buzzer_beep)
    ...
    while True:
        ...
        # Optional: prove camera staleness (for customer). Works when using GI GStreamer capture.
        if hasattr(cap, "last_frame_age_ms") and (fid % 30 == 0):
            try:
                dms_print("PERF", f"camera_frame_age_ms={getattr(cap,'last_frame_age_ms', None)}")
            except Exception:
                pass
                dms_print("WARNING", "Failed to read frame")
                break
            ...
            # YOLO Cycling: Detect continuously, alert 2 times, sleep 60s, repeat
            current_time = time.time()
            
            # Check if in sleep mode
            if current_time < yolo_sleep_until:
                yolo_active = False
            else:
                # Wake up from sleep
                if not yolo_active:
                    yolo_active = True
                    no_seatbelt_alert_count = 0  # Reset counter for new cycle
                    no_seatbelt_print_count = 0  # Reset print count for new cycle
                    seatbelt_worn_print_count = 0  # Reset worn alert count
                    buzzer_count = 0  # Reset buzzer count
                    dms_print("YOLO", "Waking up after 30s sleep - starting new detection cycle")
                
                # If we've alerted 2 times (any seatbelt status change), enter sleep mode
                if no_seatbelt_alert_count >= SEATBELT_MAX_ALERTS or seatbelt_worn_print_count >= SEATBELT_MAX_ALERTS:
                    yolo_active = False
                    yolo_sleep_until = current_time + YOLO_SLEEP_DURATION
                    dms_print("YOLO", f"2 seatbelt alerts sent - sleeping for 30 seconds")
            
            # Submit frame only if YOLO is active
            ...
            if result and getattr(result, 'multi_face_landmarks', None):
                    ...
                    if eye_closed_by_ear and iris_missing_or_low:
                        ...
                        # Eyes opened - reset duration tracking
            ...
            try:
                ...
                # DEBUG: Show current conditions for KSS override
                droop_duration_direct = 0.0
                if droop_active_since is not None:
                    droop_duration_direct = now_t - droop_active_since
                
                # ========== NEW CRITICAL OVERRIDE: Eyes Closed >2s → KSS 9 ==========
                # PRIMARY SAFETY RULE: Eyes closed for more than 2 seconds = DANGER
                # This triggers REGARDLESS of head position (faster alert, more conservative)
                ...
                if eye_close_duration > 2.0:
                    kss_score = 9.0  # Maximum severity - eyes closed
                    kss_confidence = 0.95
                    override_triggered = True
                    dms_print("KSS Override", f"🚨 CRITICAL: Eyes closed {eye_close_duration:.1f}s → KSS=9.0 (DANGER)")
                
                # ========== EXISTING CRITICAL OVERRIDE: Head droop OR eyes closed for >3s → FORCE CRITICAL (KSS 8-9) ==========
                # Two paths to critical:
                # 1. Eyes nearly closed (EAR < 0.18) AND head drooping for >3s
                # 2. Head drooping alone for >3s (even with eyes open)
                
                # Continue checking head droop logic (only if not already overridden by eyes closed)
                if not override_triggered and droop_duration_direct > 3.0:
                    # Path 1: Eyes closed + head droop
                    if avg_ear < 0.18:
                    ...
                    # Path 2: Head droop alone (eyes may be open)
                    else:
