"""
SCRFD Face Detector - IMX Board NPU Version
Uses vela-optimized model on NPU via tflite_runtime
"""
...
class SCRFDDetector:
    def __init__(self, model_path="scrfd_500m_full_int8_vela.tflite"):
        """Initialize SCRFD detector with NPU delegate"""
        
        # Load with Ethos-U NPU delegate
    ...
    def detect(self, image, conf_threshold=0.45):
        ...
        # Get all outputs and dequantize (same as PC version)
        outputs = []
        ...
        # Decode SCRFD outputs with anchor-based decoding (same as PC)
        detections = self._decode_scrfd_output(outputs, conf_threshold)
        
        # Debug output occasionally
        import random
        if random.randint(1, 30) == 1:
            print(f"[Detection-DEBUG] Found {len(detections)} faces at threshold {conf_threshold}")
        ...
        for det in detections:
            ...
            # Convert keypoints to landmarks format: [x1,y1,x2,y2,...] -> [[x1,y1],[x2,y2],...]
            kps = det['keypoints'].reshape(5, 2)
            det['landmarks'] = kps
            del det['keypoints']  # Remove keypoints key
        
        return detections
    
    def _decode_scrfd_output(self, outputs, score_threshold):
        """Decode SCRFD multi-scale anchor-based outputs"""
        # Output mapping (same order as PC version)
        ...
        # Debug: show score distribution occasionally
        import random
        if random.randint(1, 30) == 1:
            max_score = np.max(all_scores) if len(all_scores) > 0 else 0
            above_03 = np.sum(all_scores[:, 0] > 0.3)
            print(f"[Detection-DEBUG] Total anchors: {len(all_scores)}, max_score: {max_score:.3f}, above 0.3: {above_03}, above {score_threshold}: {len(filtered_boxes)}")
